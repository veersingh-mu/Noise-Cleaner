...
import { Sliders, Save, Check, AlertCircle, RefreshCw, Layers, Bell } from 'lucide-react';
import { api } from '../lib/api.js';
import { ChannelConfig, CooldownConfig } from '../lib/types.js';
...
export const ConfigEditor: React.FC<ConfigEditorProps> = ({ onSaved }) => {
  ...
  return (
    <div className="bg-surface rounded-card border border-border p-6 shadow-xl space-y-8">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-5">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Configure deduplication cooldown windows, rolling 60-second burst thresholds, and service-level alert dispatch routing.
          </p>
        </div>
        ...
        <button
          onClick={handleSave}
          disabled={isSaving}
          className="px-4 py-2 rounded bg-primary hover:bg-primary-hover text-white text-xs font-mono font-semibold flex items-center gap-2 shadow-md transition-colors disabled:opacity-50"
        >
          {isSaving ? (
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 text-xs font-mono text-slate-200">
          ...
          <h3 className="font-semibold uppercase tracking-wider text-slate-300">1. Default Cooldown Durations (Seconds)</h3>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {severities.map(sev => (
            <div key={sev} className="p-3.5 bg-background rounded border border-border space-y-1.5">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-full bg-surface border border-border rounded px-2.5 py-1.5 text-xs font-mono text-slate-100 focus:outline-none focus:border-primary"
                />
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 text-xs font-mono text-slate-200">
          ...
          <h3 className="font-semibold uppercase tracking-wider text-slate-300">2. Burst Rate Trigger Thresholds (Events / 60s)</h3>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {severities.map(sev => (
            <div key={sev} className="p-3.5 bg-background rounded border border-border space-y-1.5">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-full bg-surface border border-border rounded px-2.5 py-1.5 text-xs font-mono text-slate-100 focus:outline-none focus:border-warning"
                />
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 text-xs font-mono text-slate-200">
          ...
          <h3 className="font-semibold uppercase tracking-wider text-slate-300">3. Service Alert Channel Routing</h3>
        </div>
          ...
          <table className="w-full text-xs text-left border-collapse">
            <thead>
              <tr className="border-b border-border bg-surface-muted text-slate-400 font-mono text-[11px]">
                ...
                {channels.map(ch => (
                  <th key={ch} className="py-2.5 px-4 uppercase text-center font-mono">
                    {ch}
                  </th>
                ))}
              </tr>
